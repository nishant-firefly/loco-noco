...
def create_users_from_csv(csv_path):
    ...
    try:
        ...
        for _, row in df.iterrows():
            user_data = {"id": row["id"], "name": row["name"], "email": row["email"]}
            existing_user = db_source.read(User, row["id"])
            if not existing_user:
                db_source.create(User, user_data)
                print(f"Inserted: {user_data}")
            else:
...
if __name__ == "__main__":
    ...
    print("\nUpdating a user with ID=5...")
    update_user(5, {"name": "Updated User", "email": "updated.email@example.com"})

    print("\nDeleting a user with ID=4...")
    delete_user(4)
