...
class QueryBuilder:
    def __init__(self, 
                 mock_data_path="D:/workspace/query_builder_project/app/config/mock_data.xlsx", 
                 operations_file_path="D:/workspace/query_builder_project/app/config/operations.xlsx"):
        self.mock_data_path = mock_data_path
        self.operations_file_path = operations_file_path

        # Load mock data (auth_rules and custom_conditions)
        self.auth_rules, self.custom_conditions = self._load_mock_data()

        # Load operations data
        self.operations_data = self._load_operations_excel()

    def _load_mock_data(self):
        """Load authorization rules and custom conditions from Excel."""
        try:
            df_auth = pd.read_excel(self.mock_data_path, sheet_name='auth_rules')
            df_conditions = pd.read_excel(self.mock_data_path, sheet_name='custom_conditions')

            # Convert to dictionary
            auth_rules = df_auth.set_index('persona').to_dict(orient='index')
            custom_conditions = df_conditions.set_index('persona').to_dict(orient='index')

            return auth_rules, custom_conditions
        except Exception as e:
            raise ValueError(f"Error loading mock data from {self.mock_data_path}: {e}")

    def _load_operations_excel(self):
        """Load and process operations data from an Excel file."""
        try:
            df = pd.read_excel(self.operations_file_path)
            operations = df.to_dict(orient='records')  # Convert to a list of dictionaries
            return operations
        except Exception as e:
            raise ValueError(f"Error loading operations Excel file: {e}")
    ...
    def generate_query(persona, table, clause):
        ...
        request = {
            "operation": "select",
            "table": table,
            "columns": qb.auth_rules.get(persona, {}).get(table, ["*"]),
            "clause": clause
        }
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Display loaded operations data
        if qb.operations_data:
            print("Loaded Operations Data:")
            for operation in qb.operations_data:
                print(operation)
